...
import {
  ...
  Smile, Expand, Shrink, SpellCheck, RefreshCw, Languages, Tag, Palette, Globe, Ruler,
} from 'lucide-react'
...
export default function GenerateEmail() {
  ...
  return (
    <div className="relative">
      ...
      <div className="relative grid lg:grid-cols-[420px_1fr] gap-6 max-w-6xl">
      {/* Form */}
      <div className="space-y-6">
        <Card className="relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 to-purple-600" />
          <h2 className="font-display font-semibold text-white mb-5 flex items-center gap-2">
            <Sparkles size={18} className="text-blue-400" /> Email Details
          </h2>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="label-text flex items-center gap-1"><Palette size={11} /> Tone</label>
                <select className="input-field" value={form.tone} onChange={update('tone')}>
                  {TONES.map((t) => <option key={t} className="bg-[#161C2E]">{t}</option>)}
                </select>
              </div>
              <div>
                <label className="label-text flex items-center gap-1"><Globe size={11} /> Language</label>
                <select className="input-field" value={form.language} onChange={update('language')}>
                  {LANGUAGES.map((l) => <option key={l} className="bg-[#161C2E]">{l}</option>)}
                </select>
              </div>
              <div>
                <label className="label-text flex items-center gap-1"><Ruler size={11} /> Length</label>
                <select className="input-field" value={form.length} onChange={update('length')}>
                  {LENGTHS.map((l) => <option key={l} className="bg-[#161C2E]">{l}</option>)}
                </select>
              </div>
            </div>
...
<Card className="min-h-[500px] flex flex-col items-center justify-center relative overflow-hidden">
...
Thinking through tone, structure and details ✨
</p>
          ...
          <>
            <Card className="relative overflow-hidden">
              ...
              <textarea
                className="w-full min-h-[280px] resize-y text-sm leading-relaxed text-white bg-white/5 border border-white/10 rounded-xl p-4 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                value={fullBody()}
                onChange={(e) => {
            ...
            <Card className="relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 to-purple-600" />
              <h3 className="font-display font-semibold text-white mb-4 text-sm flex items-center gap-2">
                <Wand2 size={16} className="text-blue-400" /> AI Editing Tools
              </h3>
              <div className="flex flex-wrap gap-2">
                {editTools.map(({ action, label, icon: Icon }) => (
                  ...
                  >
                    {editing === action ? <Spinner className="border-white/30 border-t-white" /> : <Icon size={14} />}
                    {label}
                  </button>
                ))}
                <button onClick={runSuggestSubject} disabled={editing !== null} className="btn-secondary text-xs py-2 px-3.5">
                  {editing === 'suggest_subject' ? <Spinner className="border-white/30 border-t-white" /> : <Tag size={14} />}
                  Suggest Subject
                </button>
                <button onClick={() => runTranslate(form.language === 'Hindi' ? 'English' : 'Hindi')} disabled={editing !== null} className="btn-secondary text-xs py-2 px-3.5">
                  {editing === 'translate' ? <Spinner className="border-white/30 border-t-white" /> : <Languages size={14} />}
          ...
          <Card className="h-full flex flex-col items-center justify-center text-center py-24 border-dashed">
            ...
            <h3 className="font-display font-semibold text-white">Your draft will appear here</h3>
