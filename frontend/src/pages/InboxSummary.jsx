    ...
    <div className="flex flex-col items-center justify-center py-10">
      <div className="relative w-40 h-24">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-24 h-24 rounded-full bg-blue-400/30 blur-2xl animate-pulse" />
        </div>
        <svg
          viewBox="0 0 200 120"
        ...
        <span className="absolute top-1 left-6 w-1.5 h-1.5 rounded-full bg-blue-300 animate-ping" />
        <span className="absolute top-4 right-8 w-1 h-1 rounded-full bg-blue-300 animate-ping [animation-delay:0.6s]" />
        <span className="absolute bottom-8 left-10 w-1 h-1 rounded-full bg-blue-300 animate-ping [animation-delay:1.2s]" />
      </div>
      <p className="text-sm text-slate-400 mt-2 animate-pulse">Dreaming up thoughtful replies…</p>
    </div>
  )
}
    ...
    <div className="h-full flex flex-col items-center justify-center text-center py-20">
      <div className="flex items-end gap-1 h-10 mb-4">
        {[0, 1, 2, 3, 4].map((i) => (
          <span
            key={i}
            className="w-1.5 rounded-full bg-blue-400 animate-[wave_1s_ease-in-out_infinite]"
            style={{ height: '100%', animationDelay: `${i * 0.1}s` }}
          />
        ))}
      </div>
      <p className="text-sm text-slate-400">Reading your email…</p>
    </div>
  )
}

const TONE_META = {
  Formal: { Icon: Briefcase, colorClass: 'bg-blue-500/10 text-blue-400 border border-blue-500/30' },
  Friendly: { Icon: Smile, colorClass: 'bg-orange-500/10 text-orange-400 border border-orange-500/30' },
  Concise: { Icon: Zap, colorClass: 'bg-violet-500/10 text-violet-400 border border-violet-500/30' },
}
...
function ReplyPanel({ replies, onUpdate }) {
  ...
  const meta = TONE_META[active.tone] || { Icon: Inbox, colorClass: 'bg-white/5 text-slate-400 border border-white/10' }
  ...
  return (
    <div className="flex gap-3">
      <div className="flex flex-col gap-2 w-28 flex-shrink-0">
        {replies.map((r, i) => {
          const m = TONE_META[r.tone] || { Icon: Inbox, colorClass: 'bg-white/5 text-slate-400 border border-white/10' }
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs text-left transition-colors ${
                isActive ? m.colorClass : 'bg-white/5 text-slate-400 hover:bg-white/10'
              }`}
            >
              <m.Icon size={15} />
              {r.tone}
      ...
      <div className="flex-1 border-l border-white/10 pl-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-3">
            <button className="text-xs text-slate-400 hover:text-blue-400" onClick={handleCopy}>
              {copied ? 'Copied!' : 'Copy'}
            </button>
            <button className="text-xs text-slate-400 hover:text-blue-400" onClick={() => setShowActions((s) => !s)}>
...
export default function InboxSummary() {
  ...
  return (
    <div className="relative">
      <div className="absolute top-[-100px] left-[10%] w-72 h-72 rounded-full bg-blue-500 opacity-[0.18] blur-3xl animate-[drift_9s_ease-in-out_infinite] pointer-events-none" />
      <div className="absolute bottom-[-80px] right-[5%] w-80 h-80 rounded-full bg-purple-600 opacity-[0.18] blur-3xl animate-[drift_11s_ease-in-out_infinite] pointer-events-none" />

      <div className="relative grid lg:grid-cols-2 gap-6 max-w-6xl">
        <Card className="relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 to-purple-600" />
          <h2 className="font-display font-semibold text-white mb-4 flex items-center gap-2">
            <Inbox size={18} className="text-blue-400" /> Paste Email
          </h2>
          <textarea
          ...
          {result ? (
            <div className="space-y-5">
              ...
              <div className="pt-2 border-t border-white/10">
                <h3 className="font-display font-semibold text-white mb-3">Suggested Replies</h3>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center py-20 border-dashed border rounded-xl border-white/10">
              <ScanSearch size={28} className="text-slate-500 mb-3" />
